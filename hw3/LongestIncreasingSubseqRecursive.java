...
public class LongestIncreasingSubseqRecursive {
    ...
    /**
     * Longest Increasing subsequence problem using the top down approach (recursion) 
    ...
    public static int incrSubseqRecursive(int j, int[] A) {
        ...
        int maxLength = 1; 
        for (int i = 1; i < j; i++) {
            int length = incrSubseqRecursive(i, A) + 1; // recursively calculates the length of the subsequence up to the current index
